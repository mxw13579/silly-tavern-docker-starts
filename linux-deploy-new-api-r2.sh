...
cat > $PROJECT_DIR/backup-scripts/backup.sh << EOF
#!/bin/sh
...
cat > $PROJECT_DIR/backup-scripts/setup-cron.sh << 'EOF'
#!/bin/sh
...
cat > $PROJECT_DIR/restore-mysql-backup.sh << EOF
#!/bin/bash
...
if ! docker ps | grep -q mysql-backup; then
    ...
    echo -e "请确保先运行setup-mysql-r2-backup.sh脚本部署备份系统"
    exit 1
fi
